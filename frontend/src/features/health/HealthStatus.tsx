...
export function HealthStatus() {
  ...
  return (
    <Card aria-labelledby="status-title">
      <h2 className="mb-6" id="status-title">Conexão com o serviço</h2>
      {isLoading && <div className="grid justify-items-start gap-4 [&_p]:mb-0" role="status">
        <StatusBadge tone="info" label="Verificando" />
        <p>Verificando a conexão…</p>
      </div>}
      {!isLoading && error && <div className="grid justify-items-start gap-4 [&_p]:mb-0">
        <Alert className="w-full" tone="warning" role="alert">
      ...
      {!isLoading && !error && health && <div className="grid justify-items-start gap-4 [&_p]:mb-0" role="status">
